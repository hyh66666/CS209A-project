...
export default {
  ...
  methods: {
    getDataAndRenderChart1() {
      axios
          .get('http://localhost:8081/Qsan2yi') // 假设后端接口为 /api/data
          .then(response => {
            ...
            myChart.setOption({
              title: {
                text: '展示哪些 tag 的组合得到最多的 upvotes'
              },
              tooltip: {},
              xAxis: {
                data: categories,
    ...
    getDataAndRenderChart2() {
      axios
          .get('http://localhost:8081/Qsan2er') // 假设后端接口为 /api/data
          .then(response => {
            const data = response.data;
            console.log(response.data)
            // 提取折线图的数据
